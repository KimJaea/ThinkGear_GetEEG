...
int __cdecl main(int argc, char** argv) {
	if (argc < 4) {
		char path[300] = "D:/ThinkGearData/data/data.txt";
		char port[100] = "\\\\.\\COM5";
		GetEEG(300, path, port);
	}
	else {
		GetEEG(argv[1], argv[2], argv[3]);
	}
}

int GetEEG(char t[260], char path[300], char port[100]) {
	...
	if (errCode < 0) {
		fprintf(stderr, "ERROR: TG_Connect() returned %d by %s.\n", errCode, comPortName);
		wait();
		exit(EXIT_FAILURE);
	}

	/* Keep reading ThinkGear Packets from the connection for 5 seconds... */
	secondsToRun = (double)atoi(t);
